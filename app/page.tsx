'use client';
import {useEffect,useReducer,useState,useRef,useCallback,memo,type CSSProperties} from 'react';
...
import {deck,initial,reducer,shuffle,stages,type Game} from './game';
...
export default function Home(){
 ...
 const playSound=useCallback((cue:Cue)=>{audio.current??=new RetroAudio();audio.current.setEnabled(soundEnabled);void audio.current.play(cue);},[soundEnabled]);
 const flipCard=useCallback((id:number)=>{dispatch({type:'flip',id});playSound('flip');},[playSound]);
 ...
 useEffect(()=>{if(game.selected.length!==2)return;const pair=game.selected.map(id=>game.cards.find(card=>card.id===id)!);const t=setTimeout(()=>dispatch({type:'resolve'}),pair[0].fruit===pair[1].fruit?300:650);return()=>clearTimeout(t)},[game.selected]);
 useEffect(()=>{
 ...
 <CardBoard game={game} onFlip={flipCard}/>
...
const CardBoard=memo(function CardBoard({game,onFlip}:{game:Game;onFlip:(id:number)=>void}){const locked=game.phase!=='play'||game.selected.length===2;return (<div className={'board '+(game.phase==='won'?'completed':'')} style={{'--board-rows':game.cards.length/4} as CSSProperties}>
 {game.cards.map((card,index)=>{const open=game.phase==='ready'||game.phase==='preview'||game.phase==='hint'||game.selected.includes(card.id)||card.matched;return <button key={card.id} className={'card '+(open?'is-open ':'')+(card.matched?'is-matched':'')} disabled={locked||card.matched||game.selected.includes(card.id)} onClick={()=>onFlip(card.id)} aria-label={`${index+1}번 카드${open?': '+names[card.fruit]:', 뒤집기'}${card.matched?', 짝 맞춤':''}`}><span className="card-inner"><span className="card-back"><span className="back-mark">p<span>·</span></span><span className="back-label">FRUIT PAIR</span></span><span className="card-front"><svg className="fruit" viewBox={`${fruitCenters[card.fruit][0]-fruitFrameSize/2} ${fruitCenters[card.fruit][1]-fruitFrameSize/2} ${fruitFrameSize} ${fruitFrameSize}`} preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false"><image href="/fruit-atlas.webp" width="1254" height="1254"/></svg>{card.matched&&<span className="match-check"><Check size={15}/></span>}</span></span></button>})}
 </div>);});
